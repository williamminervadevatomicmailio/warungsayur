...
export default function CekPesananPage() {
  ...
  // Validate saved orders against server and remove ones that no longer exist
  useEffect(() => {
    ...
    ;(async () => {
      ...
      let changed = false
      // limit checks to first 20 to avoid spamming
  ...
  async function handleSearch(id?: string) {
    ...
    try {
      ...
      setOrder(data)
      // update saved orders list with latest data
      try {
        const key = 'my_orders'
        const existing = JSON.parse(localStorage.getItem(key) || '[]') as any[]
